...
use file_structs::{FormFile, Txtr, TxtrEntry, SondEntry};
...
pub fn main(mut args: Args, wait_for_user: bool) {
    args.audio_groups.get_or_insert_with(|| {
        (1..=4)
            .map(|i| format!("audiogroup{}.dat", i))
...
pub struct Args {
    ...
    #[structopt(long)]
    pub audio_groups: Option<Vec<String>>
}
