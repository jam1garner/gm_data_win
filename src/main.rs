...
use gm_data_win::file_structs::{FormFile, Txtr, TxtrEntry};
...
fn main() {
    ...
    if args.extract_audio {
        ...
        let _ = fs::create_dir(&sounds_folder);
    ...
    if args.mod_sprites {
        ...
        let sprites_dir = fs::read_dir(&sprites_folder).unwrap_or_else(
            |_| panic!("Missing '{}' folder", sprites_folder
        ...
        if let FormFile { txtr: Some(Txtr { offset, files, .. }), .. } = file {
            let new_files: Vec<_> = files
                .into_par_iter()
                .enumerate()
                .map(|(i, TxtrEntry { unk1, unk2, png })|{
                    let png = textures_to_edit
                        ...
                        .unwrap_or_else(|| remove_padding(png));
                    TxtrEntry {
                        unk1, unk2, png
                    }
                })
            ...
            // Write TXTR padding
            let loc = f.seek(SeekFrom::Start(file_pos as u64)).unwrap() as u32;
